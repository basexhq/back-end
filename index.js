...
const ABI_events_staging = [
	// YAGNI vs consistency ---> consistency wins :) (currently both versions of the events are the same, may change in the future)
	"event OrganisationAddedToKleros(string orgGuid, string name, address klerosAddress)",
	"event ItemAdded(string orgGuid, string orgName, string itemGuid, uint itemIndex, string itemName, string itemJSONIPFS, uint PVT, uint NVT)",
];
...
const BaseXContract_prod = new ethers.Contract(
	ADDRESS_prod,
	ABI_prod,
	provider
);
const BaseXContractEvents_prod = new ethers.Contract(
	ADDRESS_prod,
	ABI_events_prod,
	provider
);

const BaseXContract_staging = new ethers.Contract(
	ADDRESS_staging,
	ABI_staging,
	provider
);
const BaseXContractEvents_staging = new ethers.Contract(
	ADDRESS_staging,
	ABI_events_staging,
	provider
);
...
BaseXContractEvents_prod.on(
	...
	async (
		...
		console.log(
			"PROD: Event listener ---> ItemAdded ---> itemGuid ---> " + itemGuid
		);
...
BaseXContractEvents_staging.on(
	"ItemAdded",
	async (
		orgGuid,
		orgName,
		...
		console.log(
			"STAGING: Event listener ---> ItemAdded ---> itemGuid ---> " + itemGuid
		);
...
async function initialLoad_processItems() {
	...
	for (let i = 0; i < itemsContract_prod.length; i++) {
		_saveItemToDB_fetchIPFS(
			itemsContract_prod[i].itemGuid,
			itemsContract_prod[i].targetGuid,
			itemsContract_prod[i].orgIndex.toNumber(),
			itemsContract_prod[i].JSONIPFS,
			itemsContract_prod[i].PVT.toNumber(),
			itemsContract_prod[i].NVT.toNumber(),
			itemsContract_prod[i].approvedToKlerosAndTokensMinted
		);
	}
	...
	for (let i = 0; i < itemsContract_staging.length; i++) {
		_saveItemToDB_fetchIPFS(
			itemsContract_staging[i].itemGuid,
			itemsContract_staging[i].targetGuid,
			itemsContract_staging[i].orgIndex.toNumber(),
			itemsContract_staging[i].JSONIPFS,
			itemsContract_staging[i].PVT.toNumber(),
			itemsContract_staging[i].NVT.toNumber(),
			itemsContract_staging[i].approvedToKlerosAndTokensMinted,
			true
		);
	}
}
...
async function initialLoad_processOrganisations() {
	...
	for (let i = 0; i < orgsContract_prod.length; i++) {
		if (!orgsContract_prod[i].JSONIPFS) {
			console.log(
				"Organisation " +
					orgsContract_prod[i].orgGuid +
					" ---> " +
					orgsContract_prod[i].name +
					" hasn't been added to Kleros yet, skipping..."
			);
		} else {
			_saveOrganisationToDB(
				orgsContract_prod[i].orgGuid,
				orgsContract_prod[i].name,
				orgsContract_prod[i].JSONIPFS.replace("/ipfs/", ""),
				orgsContract_prod[i].klerosAddress,
				orgsContract_prod[i].payoutWallet,
				orgsContract_prod[i].PVT.toNumber(),
				orgsContract_prod[i].NVT.toNumber(),
				orgsContract_prod[i].PVThistorical.toNumber(),
				orgsContract_prod[i].NVThistorical.toNumber()
			);
		}
	}
	...
	for (let i = 0; i < orgsContract_staging.length; i++) {
		if (!orgsContract_staging[i].JSONIPFS) {
			console.log(
				"Organisation " +
					orgsContract_staging[i].orgGuid +
					" ---> " +
					orgsContract_staging[i].name +
					" hasn't been added to Kleros yet, skipping..."
			);
		} else {
			_saveOrganisationToDB(
				orgsContract_staging[i].orgGuid,
				orgsContract_staging[i].name,
				orgsContract_staging[i].JSONIPFS.replace("/ipfs/", ""),
				orgsContract_staging[i].klerosAddress,
				orgsContract_staging[i].payoutWallet,
				orgsContract_staging[i].PVT.toNumber(),
				orgsContract_staging[i].NVT.toNumber(),
				orgsContract_staging[i].PVThistorical.toNumber(),
				orgsContract_staging[i].NVThistorical.toNumber(),
				true
			);
		}
	}
...
function _saveItemToDB_fetchIPFS(
	itemGuid,
	targetGuid,
	orgIndex,
	JSONIPFS,
	PVT,
	NVT,
	approvedToKlerosAndTokensMinted,
	staging = false
) {
	...
	fetchIPFS(
		JSONIPFS.replace("/ipfs/", "").replace("{", "").replace("}", ""),
		staging
	); // Annoying Kleros formatting + me messing with one item (removing {} curly braces)
}
		...
		db.get(SQL_IPFS_get, [hash], (err, row) => {
			if (err) {
				...
				if (row) {
					...
					console.log(
						`IPFS hash ${hash} does not exist in the DB, making a GET request to Kleros IPFS node.`
					);
...
BaseXContractEvents_prod.on(
	"OrganisationAddedToKleros",
	async (orgGuid, name, klerosAddress, event) => {
		console.log(
			"Event listener ---> OrganisationAddedToKleros ---> " + orgGuid
		);
		...
		_saveOrganisationToDB(
			org.orgGuid,
			org.name,
			org.JSONIPFS.replace("/ipfs/", ""),
			org.klerosAddress,
			org.payoutWallet,
			org.PVT.toNumber(),
			org.NVT.toNumber(),
			org.PVThistorical.toNumber(),
			org.NVThistorical.toNumber()
		);
	}
);
BaseXContractEvents_staging.on(
...
function _saveOrganisationToDB(
	orgGuid,
	name,
	JSONIPFS,
	klerosAddress,
	...
	db.run(
		...
		function (err) {
			if (err) {
				...
				console.log(
					"Org ---> data processed successfully: " + orgGuid + " ---> " + name
				);
			}
		}
	);
